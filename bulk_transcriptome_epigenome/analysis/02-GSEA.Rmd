...
Running GSEA analysis for each differential expression comparison, using the [fgsea](https://www.bioconductor.org/packages/release/bioc/html/fgsea.html) package:
...
For the forebrain reference, remove clusters which are proliferating based on either labeled as such,
...
# Prep input -- function sourced from functions.R
...
# A function to add the normalized enrichment score (NES) and adj p-value to the
# enrichment plot, based on the functions included in the fgsea package
plot_enrich_with_stats <- function(sig, title, source, colour, plot_num) {
  ...
  # function sourced from functions.R
...
```{r prep_heatmap_vst, dependson = 'gene_sigs_for_vis'}
...
```{r heatmap_gene_programs_vst, dependson = 'gene_sigs_for_vis'}
